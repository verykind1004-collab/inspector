 ...
 *
 * <p>리포지토리/서비스/로그경로 정보를 런타임에 읽는다(기존 Python service_config.py 대응).
...
public class ServiceConfig {
    ...
    private volatile LogPathsBlock logPaths = new LogPathsBlock();
    ...
    /** service_config.json 을 다시 읽어 repository / services / log_paths 설정을 갱신한다. */
    public synchronized void reload() {
        if (!configFile.isFile()) {
            ...
            this.logPaths = new LogPathsBlock();
            return;
        }
        try {
            ...
            JsonNode logNode = root.get("log_paths");
            this.logPaths = (logNode == null)
                    ? new LogPathsBlock()
                    : objectMapper.treeToValue(logNode, LogPathsBlock.class);
        } catch (IOException e) {
